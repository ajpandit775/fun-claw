...
## 2026-04-29: Slice 11, Task 4 — `biome ci` is more strict than local `biome ci` at the rule level; always re-run after adding new files
Two pre-existing biome warnings remain at end of Slice 11: `useTemplate` in `system-prompt.ts` and `useOptionalChain` in `smoke-mcp-client.cjs`. Both are FIXABLE-but-flagged-as-unsafe — operand side-effects need eyeballing before applying. Defer to v0.2.0 polish.

**Process lesson from Run #1's failure:** when adding new source files (especially files that use `console.log`, even legitimately like build scripts), the local feedback loop must include `pnpm exec biome ci .`, not just `pnpm test` and `pnpm exec tsc -b`. The CI Run #1 failure caught 14 errors I didn't see locally because I'd only been running tsc and tests. **Locked rule:** before pushing any commit that adds or modifies `.mjs` / `.cjs` / `.ts` files outside `packages/cli/src/`, run `pnpm exec biome ci . > /dev/null 2>&1; echo $?` and confirm exit 0. CI's biome ci has been trustworthy at finding new lint violations; treat its output as authoritative.

## 2026-04-29: Slice 11, Task 4 — biome.json `noConsole` override uses generalized `packages/*/scripts/**` glob
The `noConsole: off` override in `biome.json` covers three globs that ALL legitimately use `console.log` for non-library purposes: `.github/scripts/**` (GitHub Actions helpers), `packages/*/scripts/**` (per-package build/verify/utility scripts), and `smoke-*.cjs` (project-root integration smokes). The `packages/*/scripts/**` glob is generalized so future packages adding build scripts don't require a biome.json edit each time — the "scripts" subdirectory name is itself a strong signal of non-library intent. CLAUDE.md's rule "Never `console.log` in library code" stays intact for everything else (including `packages/*/src/**`, where the rule fires as an error).

## 2026-04-30: Slice 11, Task 5 — v0.1.0 manual publish has no provenance attestation; v0.2.0+ via OIDC workflow does
v0.1.0 was published via `npm publish --access public` from the maintainer's local Windows machine — the **`--provenance` flag is incompatible with manual local publishing** because npm's provenance generator requires a recognized CI provider (GitHub Actions, GitLab CI, etc.). Attempting `--provenance` from a local terminal fails with `npm error code EUSAGE: Automatic provenance generation not supported for provider: null`. The supply-chain attestation we'd normally get (npm registry signs the package's source provenance with a Sigstore-rooted certificate that ties the published artifact to the GitHub Actions workflow run that built it) is **missing for v0.1.0**.

**Security model for v0.1.0**: trust the maintainer's local machine for the first publish. The published artifact carries the npm publisher's identity (ajpandit775) and 2FA-gated authentication, but no Sigstore attestation linking it back to a specific git SHA built in CI. **v0.2.0+ via the release.yml workflow restores provenance** — that workflow runs `npm publish --provenance --access public` from a GitHub-hosted Ubuntu runner with `id-token: write` and a recognized OIDC issuer, which IS supported by npm's provenance generator. Future sessions: do NOT attempt `--provenance` from a local terminal; it always fails. The flag belongs in the CI workflow only.

## 2026-04-30: Slice 11, Task 5 — `npm pkg fix` normalizes `./dist/...` bin paths and `repository.url` to npm's preferred form
npm 10+ emits a misleading warning for bin paths starting with `./` (`"bin[funclaw]" script name dist/index.js was invalid and removed`) and auto-corrects them client-side at publish time. The wording suggests a bin-stripping crisis, but the entry is actually rewritten to `dist/index.js` (only the leading `./` is stripped); the bin command survives and `npm install -g <pkg>` still produces a working binary on PATH. Similarly, `repository.url` values without the `git+` prefix and `.git` suffix get auto-normalized at publish time. Both rewrites happen client-side AFTER `npm pack` packs the tarball but BEFORE upload, so the published metadata diverges from the tarball file's package.json. **Locked workflow:** run `npm pkg fix` from each publishable package's directory BEFORE the first commit that prepares a release, so the committed package.json matches what npm actually publishes. Bin paths must be written WITHOUT the leading `./` (use `dist/index.js`, not `./dist/index.js`) and repository URLs must use the full `git+https://...git` form. Future publishes won't emit "errors corrected" warnings if the local file is already in normalized form.

## 2026-05-01: Slice 11, Task 5 — locked rule: never `npm publish` from the repo root, only from `packages/cli/`
Today's session ran `npm publish` from the repo root by mistake. npm assembled an 899KB tarball as `funclaw@0.0.0` (the workspace-root package, named `funclaw` not `fun-claw`) containing 111 files including `.claude/CLAUDE.md` (internal maintainer docs with security-incident notes), all source, all tests, and all ADRs. The publish was saved by `ENEEDAUTH` (the maintainer wasn't logged in to npm at that moment) and by `EPRIVATE` (workspace root has `private: true` as second safety net) — but neither would have stopped a leak if both were absent. **For v0.2.0+:** the `release.yml` workflow handles this automatically (the `publish-npm` job runs `npm publish` with `working-directory: packages/cli`). **For any future manual publish:** `cd packages/cli` FIRST; verify the PowerShell prompt shows `\packages\cli>` before running `npm publish`. The cd is the discipline; the prompt verification is the failsafe.

## 2026-05-01: Slice 11, Task 5 — locked rule: workspace root `package.json` must always have `private: true`
The second safety net behind "cd to `packages/cli` before publish." Belt and suspenders. Without `private: true`, an accidental `npm publish` from the repo root would attempt to publish the workspace `funclaw@0.0.0` package — containing internal docs, ADRs, tests, smoke scripts, etc. — to npm. **Removing `private: true` from the workspace root is a regression — never do it.** If a future contributor proposes removing it (e.g. "to enable workspace-level scripts"), push back: workspace scripts work fine with `private: true` set; the flag only blocks `npm publish`, which the workspace root must never do.

## 2026-05-01: Slice 11, Task 5 — PowerShell paste handling can strip newlines on multi-line copies
The chat client used in this session can lose newlines on multi-line copy operations into Windows PowerShell — multiple commands copied from a single message arrive concatenated onto a single line in the PowerShell prompt. **Discipline for the maintainer:** type commands one at a time, verify on screen before pressing Enter. Especially critical for commands containing `--` (double dashes), which become visually confusing when concatenated. **Discipline for the agent:** when walking the maintainer through PowerShell sequences, prefer one command per code-fenced block over multi-line blocks where each line is meant to be run separately. The cost of an extra fence is one render line; the cost of a concatenation accident is potentially significant.

## 2026-05-01: Slice 11, Task 5 — `npm login` browser-OAuth flow on Windows is keystroke-sensitive
`npm login` on Windows defaults to a browser-OAuth flow. The terminal prompt `Press ENTER to open in the browser...` is sensitive to extra input — pasting any extra character (even an inadvertent letter at the end of a copy-paste) crashes the prompt with `Exit handler never called.` and aborts the login flow. **Procedure:** at that prompt, press Enter, no other keystrokes. Don't paste anything, don't type a character beforehand. If the OAuth tab in the browser doesn't open, retry the whole `npm login` command rather than poking at the prompt.

## 2026-05-01: Slice 11, Task 5 — `release.yml`'s `publish-npm` job is missing the `pnpm exec tsc -b` step
The original v0.1.0 release-workflow run failed at the "Build CLI bundle" step because tsup couldn't resolve `@funclaw/*` workspace dependencies — their `dist/index.js` files weren't compiled. The `test` job in the same workflow has `pnpm exec tsc -b` between install and test, but the `publish-npm` job goes straight from `pnpm install --frozen-lockfile` to `pnpm -F fun-claw run build` (tsup), skipping the workspace-package composite-project compilation step that tsup depends on. Each workflow job runs on a fresh GitHub-hosted runner with no shared filesystem, so the `test` job's `dist/` doesn't carry over to `publish-npm`. **Fix:** add `- run: pnpm exec tsc -b` between `pnpm install --frozen-lockfile` and `Build CLI bundle` in the `publish-npm` job. (`publish-runtime-image` doesn't need this fix — its build script is `docker build`, no JS bundling, no workspace dep chain.) Applied as a separate post-v0.1.0 commit so the v0.1.0 release surface stays clean of unrelated changes. **Generalized rule:** when workflow jobs share a setup phase (install + `tsc -b` + ...), copy the WHOLE shared setup sequence between jobs, not just the install step. The publish-npm vs test job asymmetry that caused the v0.1.0 release failure was a "copied install but forgot tsc -b" instance of this broader pattern.

## 2026-05-01: Slice 11, Task 5 — empty `dist/` subdirectories from a prior `tsc -b` run aren't removed by tsup's `clean` step
Running `pnpm exec tsc -b` populates `packages/cli/dist/{commands,components}/{*.js,*.d.ts,...}` because the cli's tsconfig has `composite: true` + `declaration: true` (inherited from `tsconfig.base.json`), which emits the full source tree into the package's `outDir`. When `pnpm -F fun-claw run build` (tsup) runs next with `clean: true` on the first entry, tsup removes the FILES inside those subdirs but leaves the empty PARENT directories standing — a known Windows-rimraf quirk where directory removal can fail silently when files have been cleared but the dir handle isn't released cleanly. **Benign for publish:** `npm pack` skips empty directories, so the published tarball is unaffected (verified during v0.1.0 publish). **But worth a config hardening for v0.2.0:** add a prebuild hook (e.g. `node -e "require('fs').rmSync('dist', { recursive: true, force: true })"`) before tsup runs, so empty-dir leftovers can't accumulate. Track in v0.2.0 polish.

## 2026-05-01: Slice 12 close — v0.2.0 documentation completion (three docs deferred)
The Slice 12 docs scope was reduced from five docs to two during the v0.1.x cleanup session: `docs/getting-started.md` and `docs/faq.md` shipped; troubleshooting, skill authoring, and MCP integration deferred to v0.2.0.

**Reasoning for the deferral:** review fatigue compounded across the multi-doc block; the verification-before-apply discipline slipped twice on factual claims (the FC-5001 keyfile claim during getting-started review and the link-audit substitution during FAQ review). Troubleshooting in particular is high-risk to ship under fatigue because every error code is its own factual claim that needs source verification — a wrong fix for FC-5001 is worse than no troubleshooting doc at all. Two verified-to-source docs is a better v0.1.x deliverable than five docs of decreasing review quality.

**The three deferred docs to write in v0.2.0:**
1. `docs/troubleshooting.md` — FC-xxxx error code reference. Each code mapped to cause and fix. Sources: `packages/core/src/config.ts` (FC-5xxx keyfile codes), `packages/core/src/agent-loop.ts` (FC-2xxx provider codes via the provider adapters), `packages/docker-runner/src/policy.ts` and `runner.ts` (FC-1xxx sandbox codes), `packages/mcp-client/src/errors.ts` (FC-3xxx MCP codes), `packages/skills/src/errors.ts` (FC-4xxx skill codes), `packages/core/src/tools/spawn-subagent.ts` (FC-6xxx subagent codes). Every code claim must be source-cited.
2. `docs/skill-authoring.md` — how to write a SKILL.md, frontmatter schema, the discovery directories per OS, the bundled vs user vs project source precedence, examples. Sources: `packages/skills/src/parser.ts`, `packages/skills/src/discovery.ts`.
3. `docs/mcp-integration.md` — config TOML `[mcp.<name>]` shape, supported transports (stdio only at v0.1.0; HTTP/SSE flagged `[v2-or-never]` in saved feedback), tool-name namespacing (`mcp__<server>__<tool>`), what an MCP server needs to expose to be useful, debugging connection issues. Sources: `packages/mcp-client/`, the existing `smoke-mcp-*` scripts at the repo root, sample server configs.
